...
이 에러를 추적하면서 배운 것이 두 가지 있다.

첫째, **Reactor 스택 트레이스에서 보이는 예외가 원본이 아닐 수 있다.** 에러 핸들러가 에러 응답을 렌더링하다가 2차 예외를 던지면 원본 예외는 마스킹된다. 500이 찍혔다고 비즈니스 로직 버그라고 단정하면 원인을 찾기 어렵다.

둘째, **"thread-safe하지 않다"는 것은 "터질 수도 있고 안 터질 수도 있다"는 뜻이다.** `ArrayList`의 동시 접근은 항상 문제를 일으키는 게 아니라 CPU 캐시 타이밍이 겹칠 때만 발생한다. 재현이 안 되고 로그만 봐서는 감을 잡기 어려운 유형의 버그다. 프레임워크 내부의 자료구조가 thread-safe한지까지 의심해야 하는 순간이 올 수 있다.